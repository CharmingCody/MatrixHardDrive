...
The internal convention is following. The objects are kept in the tree structure that you can visualize in the external software - we use the popular worldwide established and opened file formats. You can also visualize that structure in the F12 console, when you type
window.mhd.carModel
...
Due to webbrowsers security mechanisms and necessity to load quickly and comfortably many files and modules, the most convenient way is to run MatrixHardDrive from the local webserver. For now, when there is no multiplayer mode it can be any HTTP webserver, even the simplest.

For example running it with the Apache2 webserver: after installing the Apache2 webserver, the usual website folder is:
/var/www/html
...
In the future more convenient configurable interface will be introduced for the joystics and keyboard, mouse configuration. Nevertheless, since everything is possible from the code, we focus on developing possibly quick new game functionalities.
